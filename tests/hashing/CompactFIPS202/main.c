 ...
 *
 * @file
 * @brief       Application for benchmarking the performance of Keccak-1600 (CompactFIPS202)
 *
...
const unsigned long long int databytelens[] = { 64, 100, 1024, 10240 };
...
void *keccak1600_hash(void *args) {
    ...
    Keccak_init(keccak1600_args->keccak_state, keccak1600_args->rate, 1600-keccak1600_args->rate, keccak_delimited_suffix);
    Keccak_update(keccak1600_args->keccak_state, keccak1600_args->input, keccak1600_args->inputByteLen);
    Keccak_final(keccak1600_args->keccak_state, keccak1600_args->output, hashbitlen/8);
...
void keccak_benchmark(size_t databytelen, char* datastring, uint16_t p) {
    ...
    const unsigned char* data = (unsigned char*) datastring;

    /* Measure performance of Keccak */
    printf("Measure performance of Keccak-%u.\n", p);
    unsigned char keccak_hashval[hashbitlen/8];
    int keccak_rates[5];
    ...
    for (uint32_t j=0; j<5; j++) {
        ...
        keccak_state_t keccak_state_instance;
        keccak1600_struct args = {
                                    .keccak_state = &keccak_state_instance,
                                    .rate = keccak_rates[j],
                                    .input = data,
                                    .inputByteLen = databytelen,
                                    .output = keccak_hashval
                                };
        ...
        hash_thread_pid = thread_create(hash_thread_stack, sizeof(hash_thread_stack),
                                    ...
                                    keccak1600_hash, &args, "CompactFIPS202");
